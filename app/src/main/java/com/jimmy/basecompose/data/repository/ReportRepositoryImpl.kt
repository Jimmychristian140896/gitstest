...
class ReportRepositoryImpl(
    ...
    override suspend fun getReports(
        ...
        val result = safeCall<BasePagingResponse<ReportDto>> {
            client.get(
                urlString = constructRoute("/reports")
            ) {
                search?.let {
        ...
        when(result) {
            ...
            is Result.Success -> {
                ...
                if(saveToLocal) {
                    articleDao.deleteArticlesByType(ArticleType.REPORT)
                    articleDao.upsertArticles(articles.map { it.toArticleEntity() })
                }
                return Result.Success(result.data.results.map { it.toArticle() })
            }
        }
    }
    override suspend fun getReportById(id: Int): Result<Article, DataError> {
        val result = safeCall<ReportDto> {
            client.get(
                urlString = constructRoute("/reports/$id")
            ) {
    ...
    override suspend fun getLastLoadedReports(): Result<List<Article>, DataError> {
        val articles = articleDao.getArticlesByType(ArticleType.REPORT).map { it.toArticle() }
